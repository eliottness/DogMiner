...
import com.softalys.metrics.tasks.MetricCollector;
import com.softalys.metrics.tasks.MetricCollectors;
    ...
    public SendMetricsTask( boolean sync )
    {
        ...
        for( MetricCollectors collector : MetricCollectors.values() )
        {
            if( collector.isSync() == this.isSync() )
            {
                ...
                {
                    DogMinerPlugin.getInstance().getLogger().log( Level.SEVERE, "Could not initialize collector: " + collector.name(), ex );
                }
            }
        }
    }
